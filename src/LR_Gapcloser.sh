...
echo "  -l    the error-corrected long reads used to close gaps. The file should be                           "
echo "        fasta format.                                                               [         required ]"
...
if [ $NUMARGS == 0 ] 
  ...
  while getopts ":i:l:t:c:a:m:n:g:v:r:o:" opt;
  do  
    case $opt in
        i)contigs=$OPTARG 
          con=1 ;;
...
echo "-i(scaffolds)=$contigs -l(longread)=$longread -t(thread)=$thread -c(coverage)=$coverage -a(tolerance)=$tolerance -m(max_distance)=$max_distance -n(number)=$number -g(taglen)=$taglen -v(overstep)=$overstep -o(output)=$output"
  if [ ! -d $output ] ; then
    mkdir "$output" 
  else
  ...
  perl $program/tag_generator.pl $output/LR.fasta $taglen >$output/iteration-1/tmp/LR.tag.fa
  perl $program/split.pl $output/iteration-1/tmp/LR.tag.fa $number $output/iteration-1/tmp pro_
  rm $output/iteration-1/tmp/LR.tag.fa
      for  FILE in $output/iteration-1/tmp/pro_*
        do
          $program/bwa mem -k17 -W40 -r10 -A1 -B1 -O1 -E1 -L0 -t $thread $output/iteration-1/db/genome $FILE >$FILE.mem 2>>$output/iteration-1/tmp/mem.error
          rm $FILE
        done

        for FILE in $output/iteration-1/tmp/pro_*.mem
            do
              perl $program/coverage_calculator.pl $FILE $FILE.coverage &
            done
        wait
        cat $output/iteration-1/tmp/pro_*.mem.coverage >$output/iteration-1/tmp/tag_coverage
        rm $output/iteration-1/tmp/pro_*;
...
#### iteration process
  ...
  perl $program/tag_generator.pl $output/LR.fasta $taglen >$output/iteration-$j/tmp/LR.tag.fa
  perl $program/split.pl $output/iteration-$j/tmp/LR.tag.fa $number $output/iteration-$j/tmp pro_
  rm $output/iteration-$j/tmp/LR.tag.fa
      for  FILE in $output/iteration-$j/tmp/pro_*
        do
          $program/bwa mem -k17 -W40 -r10 -A1 -B1 -O1 -E1 -L0 -t $thread $output/iteration-$j/db/genome $FILE >$FILE.mem 2>>$output/iteration-$j/tmp/mem.error 
          rm $FILE
        done
        for FILE in $output/iteration-$j/tmp/pro_*.mem
            do
              perl $program/coverage_calculator.pl $FILE $FILE.coverage &
            done
        wait
        cat $output/iteration-$j/tmp/pro_*.mem.coverage >$output/iteration-$j/tmp/tag_coverage
        rm $output/iteration-$j/tmp/pro_*;
...
rm $output/LR.fasta $output/genome_uc.fasta $output/LR_length
else 
  usage
fi
fi
